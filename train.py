...
def main(parser_data):
    ...
    # Directories
    save_dir = Path(parser_data.output_dir) / "exp_{}".format(datetime.datetime.now().strftime("%Y_%m_%d_%H_%M_%S"))
    ...
    data_transform = {
        "train": transforms.Compose([transforms.RandomSampleCrop(),
                                     transforms.ToTensor(),
                                     transforms.RandomHorizontalFlip(0.5)]),
        "val": transforms.Compose([transforms.ToTensor()])
    }

    base_root = parser_data.data_path
    # check dataset root
    if os.path.exists(os.path.join(base_root, "metadata")) is False:
        raise FileNotFoundError("metadata dose not in path:'{}'.".format(base_root))

    # load train data set
    train_dataset = YOLODataSet(base_root, data_transform["train"], "tr_list.txt")
    train_sampler = None
    ...
    val_dataset = YOLODataSet(base_root, data_transform["val"], "val_list.txt")
...
if __name__ == "__main__":
    ...
    # 训练数据集的根目录(VOCdevkit)
    parser.add_argument('--data-path', default='/home/xuyufeng/projects/python/yolov5/datasets/', help='dataset')
    # 检测目标类别数(不包含背景)
