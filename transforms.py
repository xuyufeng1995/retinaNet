...
class Resize(object):
    """对图像进行resize处理,target中目标框位置也相应变化，该方法应放在ToTensor前"""
    def __init__(self, size=(1088, 640), scale_up=True, color=(114, 114, 114)):
        self.size = size
    ...
    def __call__(self, image, target):
        ...
        # Compute padding
        new_unpad = int(round(shape[1] * r)), int(round(shape[0] * r))  # height, width
        dw, dh = self.size[0] - new_unpad[1], self.size[1] - new_unpad[0]  # wh padding

        dw /= 2  # divide padding into 2 sides
        dh /= 2

        if shape[::-1] != new_unpad:  # resize
            image = t.Resize(new_unpad)(image)

        top, bottom = int(round(dh - 0.1)), int(round(dh + 0.1))
        left, right = int(round(dw - 0.1)), int(round(dw + 0.1))
        image = ImageOps.expand(image, border=(left, top, right, bottom), fill=self.color)  # add border
        target["boxes"] = target["boxes"] * r + torch.tensor([dw, dh, dw, dh]).reshape(1, -1).repeat(target["boxes"].shape[0], 1)
...
class PlotImage(object):
    ...
    def __call__(self, image, target):
        if self.first and is_main_process:
            # plot image
            image_array = image.numpy()  # 将tensor数据转为numpy数据
            image_array = image_array * 255 / image_array.max()  # normalize，将图像数据扩展到[0,255]
            mat = np.uint8(image_array).transpose(1, 2, 0)
            mat = cv2.cvtColor(mat, cv2.COLOR_BGR2RGB)

            for box in target["boxes"]:
                cv2.rectangle(mat, (int(box[0]), int(box[1])), (int(box[2]), int(box[3])), color=(0, 0, 255))
            cv2.imwrite(self.save_dir + "/labels.jpg", mat)
            self.first = False
    ...
    def __call__(self, image, target):
        ...
        while True:
            ...
            for _ in range(5):
                ...
                if w/h < 1 or w/h > 2:  # 保证宽高比例在0.5-2之间
                    continue
